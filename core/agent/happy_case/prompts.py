...
2. **input_data**: 
   - 1. **컨텍스트 분석 및 데이터 추출**: `<BUSINESS_LOGIC>`을 분석하여 인증 토큰(`Authorization` 헤더 등), `@PathVariable`, `@RequestParam` 등 필수 파라미터를 식별합니다.
...
3. **expected_result**: 
   - 성공 시 예상되는 응답 데이터를 작성하세요. (의미 있는 헤더가 있다면 JSON 코드 블록 위에 명시)
   - 아래와 같은 마크다운 리스트 형식으로 작성합니다.
   ...
   (의미 있는 헤더가 없을 경우 Headers 항목은 생략 가능, 응답 바디가 없을 경우 Body 값에 `None` 표기)
"""
