    ...
    def trace_roots(self, state: AgentState) -> Dict[str, Any]:
        ...
        for target in targets:
            ...
            for row in paths:
                ...
                # Fetch parameters for the root method context
                param_query = """
                ...
                """
                params_result = self.db_client.execute_query(param_query, {"root_id": root_node.element_id}) # Neo4j python driver specific
                ...
                trace_results.append(TraceResult(
                    root_method_id=root_node.element_id,
                    root_method_signature=root_node.get('signature', ''),
                    root_method_code=root_node.get('source', ''),
                    target_method_id=target_node.element_id,
                    target_method_code=target_node.get('source', ''),
                    call_path=call_path,
                    parameters=parameter_infos
                ))

        logger.info(f"Traced {len(trace_results)} unique paths.")
        # Remove duplicates if any (based on root + target combo)
        # For now, keep all to see multiple paths.
        return {"trace_results": trace_results}

    def generate_scenarios(self, state: AgentState) -> Dict[str, Any]:
        """
        Generate test scenarios using LLM based on trace results.
        """
        traces = state.trace_results
        generated_scenarios = []
        
        logger.info(f"Generating scenarios for {len(traces)} traces...")
        ...
        for i, trace in enumerate(traces):
            try:
                ...
                inputs = {
                    "root_method": f"Signature: {trace.root_method_signature}\nCode:\n{trace.root_method_code[:2000]}...", # 2000자 제한
                    "modified_method": f"Code:\n{trace.target_method_code[:2000]}...",
                    "call_path": " -> ".join(trace.call_path),
                ...
                if isinstance(result, list):
                    for item in result:
                        scenario = GeneratedScenario(
                            test_case_id=f"TC-{i+1:03d}", # Temporary ID
                ...
                logger.info(f"Generated scenarios for trace {i+1}/{len(traces)}")
                
            except Exception as e:
                logger.error(f"Failed to generate scenario for trace {i}: {e}")
                # state.errors.append(...) # Cannot mutate directly if returning dict updates
